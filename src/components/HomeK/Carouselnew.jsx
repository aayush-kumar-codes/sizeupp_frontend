    ...
    <>
      <div className="relative h-screen ">
        {BannerData.map((banner, index) => (
          <div
            key={banner.id}
            className={`absolute top-0 left-0 w-full h-full bg-black  ${index === currentBanner ? 'transform translate-x-0' : 'transform -translate-x-full'}`}
          >
            <img
              src={banner.img}
              alt={`Banner ${banner.id}`}
              className={`object-cover hidden md:block w-full h-full ${index === currentBanner ? '' : ''
                }`}
            />
            <img
              src={banner.smimg}
              alt={`Banner ${banner.id}`}
              className={`object-cover md:hidden block w-full h-full ${index === currentBanner ? '' : ''
                }`}
            />
        ...
        <div className="absolute bottom-8 left-1/2 transform -translate-x-1/2 flex space-x-2">
          {BannerData.map((_, index) => (
            <button
              ...
              className={`w-2 h-2 rounded-full bg-white opacity-50 focus:outline-none ${index === currentBanner ? 'opacity-100 scale-110' : ''
                }`}
            ></button>
          ))}
        </div>
